...
#     _init_global()  # Initialize _args
#     model = OFU(in_channels=1, out_channels=1,scale=8).cuda()
#
#
#     # Create a random input tensor to simulate input data
#     input_tensor = torch.randn(1, 1, 32, 32).cuda()  # Input shape: [batch_size, channels, height, width]
#
...
class CAGSNet(nn.Module):
    ...
    def __init__(self, classes=19, M=3, N=21, dropout_flag=False):
        ...
        # self.CARAFE   = CARAFE(128, 128)
        self.EUCB = EUCB(in_channels=1, out_channels=1)
            # _init_global()  # Initialize _args
        _init_global()
        self.OFU = OFU(in_channels=1, out_channels=1,scale=8)
