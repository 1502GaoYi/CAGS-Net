...
class DepthWiseConv2d(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size=3, padding=1, stride=1, dilation=1):  #初始化
        super().__init__()
...
class LayerNorm(nn.Module):
    ...
    """
    def __init__(self, normalized_shape, eps=1e-6, data_format="channels_last"):  #初始化
        super().__init__()
...
#GAB模块
'''
首先要对齐高级（High-level）特征图和低级（Low-level）特征图，包括通道对齐和宽高对齐。
具体来说，就是对高级特征图进行降维和上采样（对应上图的CBS操作），低级特征保持不动。对齐后分别对高级特征图和低级特征图平均切分（Split）成4组。
然后，Mask作为一个通道的张量（上图的紫色）复制4份后和高级特征、低级特征组合到一起得到4组张量，
再分别使用不同扩张率（对应上图的d=7、d=5、d=2、d=1）的扩张卷积对这4组张量进行特征融合，融合后的4组张量再拼接到一起。
代码中最后还接了一个PW，既用来融合不同组合的特征，也起到了降维的作用（通道数变成跟低级特征图一样的通道数）。
'''
class group_aggregation_bridge(nn.Module):
    def __init__(self, dim_xh, dim_xl, k_size=3, d_list=[1,2,5,7]):   #初始化
        super().__init__()
    ...
    def forward(self, xh, xl, mask):   #前向传播
...
class Grouped_multi_axis_Hadamard_Product_Attention(nn.Module):
    def __init__(self, dim_in, dim_out, x=8, y=8):   #初始化
        super().__init__()
...
if __name__ == "__main__":
    ...
    # 假设模型和输入已定义
    model = EGEUNet(num_classes=1, input_channels=3, c_list=[8,16,24,32,48,64], bridge=True, gt_ds=True).cuda()
